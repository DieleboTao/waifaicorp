// server.cjs – Full WaifaiCorp backend
...
if (!SUPABASE_URL || !SUPABASE_ANON_KEY || !GROQ_API_KEY) {
  console.error("❌ Missing Supabase or Groq keys");
  process.exit(1);
}
...
// In-memory stores
const sessions = new Map();        // assistant sessions
const otpStore = new Map();        // email -> { code, expires }
const signupOtps = new Map();      // email -> { code, expires, companyName, password }
...
app.post('/api/send-signup-otp', async (req, res) => {
  ...
  try {
    await resend.emails.send({
      ...
      html: `<p>Your verification code is: <strong>${code}</strong></p><p>Enter this code to complete your account creation. Valid for 10 minutes.</p>`
    });
...
// ============ SIGNING OTP (for contract signers) ============
...
app.post('/api/ai/assistant', async (req, res) => {
  try {
    ...
    // All answers collected – generate contract
...
app.get('/api/contracts/:contractId/pdf', async (req, res) => {
  try {
    ...
    if (contract.status !== 'signed') {
      return res.status(403).json({ error: 'Contract not signed yet. Download only after signing.' });
    }
...
// ============ DIRECT SIGNING (with OTP, selfie, email, parties update) ============
...
app.post('/api/client/sign-direct/:qrToken', async (req, res) => {
  ...
  res.json({ success: true, contractId: contract.id, status: newStatus });
});
