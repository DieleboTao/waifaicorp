// server.cjs – WaifaiCorp with Brevo Email (and optional SMS)
...
if (!BREVO_API_KEY) {
  console.warn("⚠️ BREVO_API_KEY not set. Emails will NOT be sent. Set it in Render environment.");
}
...
async function sendEmail(to, subject, htmlContent) {
  if (!brevoEmailApi) {
    console.warn(`📧 EMAIL NOT SENT (no API key): to=${to}, subject=${subject}`);
    return false;
  }
  try {
    const sendSmtpEmail = new Brevo.SendSmtpEmail();
    sendSmtpEmail.to = [{ email: to }];
    sendSmtpEmail.sender = { email: 'waifaicorp@brevo.com', name: 'WaifaiCorp' };
...
// ============ HELPER: Send SMS (optional) ============
async function sendSMS(phoneNumber, message) {
  if (!brevoSmsApi) {
    console.warn(`📱 SMS NOT SENT (no API key): to=${phoneNumber}`);
    return false;
  }
  try {
    const sendSms = new Brevo.SendSms();
    sendSms.type = 'transactional';
    sendSms.sender = 'WaifaiCorp'; // max 11 chars
    sendSms.recipient = phoneNumber;
    sendSms.content = message;
    sendSms.unicodeEnabled = true;
    await brevoSmsApi.sendTransacSms(sendSms);
...
app.post('/api/contracts', async (req, res) => {
  try {
    ...
    // Send signing invitation emails to all parties
    const signingLink = `${BASE_URL}/client-view.html?token=${qrToken}`;
    for (const party of parties) {
      if (party.email) {
        const emailHtml = `
          <div style="font-family: Arial, sans-serif; max-width: 600px;">
            <h2>You have been invited to sign a contract</h2>
            <p><strong>Contract:</strong> ${title}</p>
            <p><strong>Your role:</strong> ${party.role}</p>
            <a href="${signingLink}" style="background: #0B2B4B; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px;">Sign Contract Now</a>
            <p>Or copy this link: ${signingLink}</p>
          </div>
        `;
    ...
    // Notify the company that created the contract
    const { data: company } = await supabase.from('companies').select('email').eq('id', companyId).single();
    if (company?.email) {
      await sendEmail(company.email, `Contract created: ${title}`, `<p>Your contract "${title}" has been created. Signing invitations sent to all parties. <a href="${BASE_URL}/dashboard.html">Go to dashboard</a></p>`);
    }
...
app.listen(PORT, () => {
  ...
  if (BREVO_API_KEY) console.log(`📧 Email service active (Brevo)`);
